...
| **QGIS Publisher** | `/publish-qgis` | **Deterministic QGIS plugin release packager & manager**. Validates `metadata.txt` against INI interpolation bugs, enforces single-root folder zip structure, manages SemVer bumps, and preps packages for plugins.qgis.org. | Eliminates plugin portal upload rejections, `%` interpolation crashes, and package structure errors. |
